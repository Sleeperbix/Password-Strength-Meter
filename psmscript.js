...
function checkPassword() {
    ...
    document.getElementById("entropy").innerText = `Entropy: ${entropy.toFixed(2)} bits`;
...
function CharacterSets(password) {
    ...
    if (/[a-z]/.test(password)) {size += 26; lowC.classList.add("active");}
    if (/[A-Z]/.test(password)) {size += 26; uppC.classList.add("active");}
    if (/[0-9]/.test(password)) {size += 10; numb.classList.add("active");}
    if (/[\W_]/.test(password)) {size += 33; spec.classList.add("active");}
    return size;
}
...
function getStrength(password, entropy) {
    ...
    if (entropy > 120) score = 5, feedback.push("Password Strength: Very Strong ");
    else if (entropy > 90) score = 4, feedback.push("Password Strength: Strong");
    else if (entropy > 60) score = 3, feedback.push("Password Strength: Moderate");
    else if (entropy > 30) score = 2, feedback.push("Password Strength: Weak");
    else if (entropy > 0) score = 1, feedback.push("Password Strength: Very Weak");
...
function updateStrengthBar(score) {
    ...
    let classes = ["empty", "veryweak", "weak", "moderate", "strong", "verystrong"];
...
async function checkHaveIBeenPwned() {
    ...
    try {
        ...
        hibpFeedback.innerHTML = "<span style='color: orange;'>Error checking HIBP.</span>";
    }

    if (breached == 1){
        hibpFeedback.innerHTML = "<span style='color: red;'>This password has been breached <b>"+ breachCount + "</b> times!</span>";
        strengthFeedback.innerHTML = "<span style ='color: red;'>Breached Password";
        strengthFill.style.width = "100%";
        strengthFill.className = "strength-fill breached";
    }
    else {
        hibpFeedback.innerHTML = "<span style='color: green;'>This password has been breached <b>"+ breachCount + "</b> times!</span>";
    }
